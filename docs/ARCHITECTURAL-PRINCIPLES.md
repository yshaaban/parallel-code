...
This is the source of truth for Parallel Code architecture rules.
...
3. bug fixes
4. test strategy
5. whether a change fits the repo's direction

`docs/ARCHITECTURE.md` explains how the system works today. This document explains where new
behavior belongs.
...
Parallel Code has many moving parts:
...
The system stays maintainable when ownership is explicit.

The core rule is simple:

**Every important domain should have one clear owner, one transport boundary, and one presentation layer.**

## Core Principles

### 1. Backend owns real external state

If state depends on the filesystem, git, PTYs, process execution, preview reachability, or
server-side observation, the backend is the authority.
...
The renderer may project this state, but it must not redefine it. Cross-client collaboration state
follows the same rule: peer presence, control ownership, and takeover outcomes come from the
backend or browser control plane, not from whichever tab rendered first.
...
The renderer must not become the hidden source of truth for backend-owned domains.
...
If a component starts deciding durable task semantics, review freshness, or backend truth, the
ownership is probably wrong. Dialogs, banners, and chips may present takeover flows, but they must
not own timeout policy, approval rules, or canonical controller state.

### 6. Persistence and restore must be explicit and exact

Startup and restore bugs usually come from fuzzy identity.
...
Electron, browser/server mode, and remote/mobile mode may adapt runtime behavior. They must not
...
Do not scatter lifecycle policy across handlers, stores, and components.
...
That is fine as long as those heuristics do not become canonical truth for tasks, supervision,
review, or git state. A tab may fit its own xterm locally; it must not assume that local fit grants
authority to resize a
...
- add temporary restore shortcuts that bypass exact identity
...
If any answer is unclear, the code is probably in the wrong place.
...
- Start here for ownership rules:
  - `docs/ARCHITECTURAL-PRINCIPLES.md`
- Use this for the current runtime map:
