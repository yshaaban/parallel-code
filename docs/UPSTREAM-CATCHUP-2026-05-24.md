...
| `cb0e886` InfoBar allowOverflow for add-agent menu        | adopt-rewrite | Follow-up local parity restored the add-agent menu in the task AI terminal `InfoBar`, so local `InfoBar` now supports opt-in overflow for that menu without changing ordinary clipped status bars.                                                              |
| `b57d6d5` side-by-side multi-agent task terminals         | adopt-rewrite | Follow-up local parity keeps the browser-first layout owner (`TaskAiTerminalSection`) but restores the missing multi-agent management surface: add-agent menu, selectable agent tabs, close-agent controls, and selected-agent restore through collapse/reopen. |
