...
3. Shortcut/keybinding behavior integrated with local shortcut owners; the full upstream
   user-configurable keybinding system is deferred pending a browser-first design.
...
6. Existing-worktree import and PR check watching, both deferred as backend-owned feature
   redesigns.
7. Optional product decisions: coverage radar, non-git projects, MiniMax provider, Docker runner
   profile inspiration, and theme variants are not direct-port candidates in this pass.
...
| `16bc331` MiniMax Ask Code provider                       | redesign      | Deferred product/provider decision; add through backend Ask Code provider abstraction and secret/config ownership if adopted.                              |
| `88e88e3` existing worktree import                        | redesign      | Do not direct-port renderer-owned import flow; future version must be backend-owned task registration with review/git/steps watchers and browser replay.   |
...
| `5330889` configurable keybinding system                  | redesign      | Full user-configurable keybindings are deferred; local pass ports useful shortcuts through `src/lib/shortcuts.ts` and `src/runtime/app-shortcuts.ts` only. |
...
| `9766235` PR CI checks                                    | redesign      | Deferred; useful feature must be backend-owned, replayable CI state rather than Electron window-event polling.                                             |
...
| `51eb740` coverage radar                                  | redesign      | Deferred optional review signal; backend must own coverage parsing/config and push projection if adopted.                                                  |
...
| `cbdc770` commit history navigation direct mode           | redesign      | Deferred with changed-files navigation redesign; do not reintroduce upstream commit-nav IPC shape into local review owners.                                |
...
- Defer full user-configurable keybindings until there is a browser-first persistence/replay
  design; port only concrete shortcut behavior through local shortcut policy.
...
- Existing-worktree import: deferred backend-owned redesign, because upstream's renderer-owned
  imported-task flag would bypass local watcher/replay/task lifecycle ownership.
- PR CI status watcher: deferred backend-owned redesign with replayable state and browser bootstrap.
- Coverage radar, non-git projects, MiniMax provider, and task-container runner-profile ideas:
  explicit future product decisions; no direct upstream file-shape port in this pass.
