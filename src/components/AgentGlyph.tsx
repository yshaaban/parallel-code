...
type AgentGlyphKind = 'claude' | 'codex' | 'gemini' | 'generic' | 'hydra' | 'opencode';
...
function normalizeAgentGlyphKind(agentDef: AgentDef | null | undefined): AgentGlyphKind {
  if (!agentDef) {
    return 'generic';
  }

  const normalizedId = agentDef.id.trim().toLowerCase();
  const normalizedName = agentDef.name.trim().toLowerCase();
  const normalizedCommand = agentDef.command.trim().toLowerCase();
  const haystack = `${normalizedId} ${normalizedName} ${normalizedCommand}`;
...
function getGlyphPalette(kind: AgentGlyphKind): GlyphPalette {
  switch (kind) {
    case 'claude':
      return {
        background: 'color-mix(in srgb, #f4a36a 18%, transparent)',
        border: 'color-mix(in srgb, #f4a36a 40%, transparent)',
        stroke: '#f0b07e',
        accent: '#ffd8bb',
      };
    case 'gemini':
      return {
        background: 'color-mix(in srgb, #7eb6ff 18%, transparent)',
        border: 'color-mix(in srgb, #7eb6ff 42%, transparent)',
        stroke: '#8fd0ff',
        accent: '#d7f0ff',
      };
    case 'codex':
      return {
        background: 'color-mix(in srgb, #78dcb4 18%, transparent)',
        border: 'color-mix(in srgb, #78dcb4 40%, transparent)',
        stroke: '#77e2bf',
        accent: '#d9fff1',
      };
    case 'opencode':
      return {
        background: 'color-mix(in srgb, #f59cff 16%, transparent)',
        border: 'color-mix(in srgb, #f59cff 38%, transparent)',
        stroke: '#f4b1ff',
        accent: '#fff0ff',
      };
    case 'hydra':
      return {
        background: 'color-mix(in srgb, #b19cff 18%, transparent)',
        border: 'color-mix(in srgb, #b19cff 40%, transparent)',
        stroke: '#cabdff',
        accent: '#f2eeff',
      };
    case 'generic':
      return {
        background: `color-mix(in srgb, ${theme.fgSubtle} 14%, transparent)`,
        border: `color-mix(in srgb, ${theme.fgSubtle} 26%, transparent)`,
...
function getAgentGlyphLabel(agentDef: AgentDef | null | undefined): string {
  if (!agentDef) {
    return 'Unknown agent';
  }

  const trimmedName = agentDef.name.trim();
  if (trimmedName.length > 0) {
    return trimmedName;
  }

  const trimmedId = agentDef.id.trim();
