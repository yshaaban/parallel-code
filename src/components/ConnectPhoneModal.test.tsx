...
describe('ConnectPhoneModal', () => {
  ...
  it('shows existing pushed browser remote status without starting a server', async () => {
    ...
    expect(screen.getByRole('button', { name: 'https://wifi' })).toBeDefined();
    expect(screen.getByRole('button', { name: 'Close' })).toBeDefined();
  });

  it('closes through the shared dialog Escape handling', async () => {
    isElectronRuntimeMock.mockReturnValue(false);
    setStore('remoteAccess', {
      enabled: true,
      connectedClients: 0,
    ...
    expect(await screen.findByRole('dialog', { name: 'Server Access' })).toBeDefined();
