...
import { registerAllHandlers, type RegisteredIpcRuntime } from './ipc/register.js';
import {
  snapshotTaskNotesWriterEntitlements,
  type TaskNotesWriterEntitlements,
} from './ipc/task-notes-writer-entitlements.js';
...
import { stopAgentRunnersAfterTaskExperience } from './ipc/task-experience-runtime-composition.js';
...
let mainWindowRuntime: RegisteredIpcRuntime | null = null;
...
export interface ElectronApplicationCompositionOptions {
  readonly taskNotesWriterEntitlements?: TaskNotesWriterEntitlements;
}

function createWindow(options: ElectronApplicationCompositionOptions): void {
...
/** Side-effect-free until called, so a trusted alternate entry can compose one proven writer. */
export function startElectronApplication(
  options: ElectronApplicationCompositionOptions = {},
): void {
  if (applicationStarted) throw new Error('Electron application composition already started');
  ...
  app.on('before-quit', (event) => {
    ...
    const runtimeCleanup = settleDesktopRuntimeCleanupOwners([
      { cleanup: windowRuntimeCleanup, label: 'window runtime' },
      { cleanup: agentRunnerFallbackCleanup, label: 'agent runner' },
