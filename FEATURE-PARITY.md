...
The largest gap from the original review is now partially closed in Parallel Code:
...
This report still applies, but read any browser-preview gap described as "missing" as "first-pass implemented, still behind AgentRove in preview/mobile/IDE breadth and sandbox isolation."

## Scope

This report compares AgentRove (`/tmp/agentrove`) against Parallel Code (`/home/yrsh/parallel-code`) and identifies high-priority features that Parallel Code should consider adding.
...
1. **Project-level diff and review workspace**
   AgentRove has a dedicated repo-wide diff view with staged/unstaged/branch/all modes plus a file tree. Parallel Code has a changed-files list and single-file diff modal, but no multi-file review surface.
   Priority: `High`
   Difficulty: `Large`
...
3. **Embedded browser, IDE, and VNC-style task surfaces**
   AgentRove gives users a unified workbench: terminal, file tree, diffs, browser preview, mobile preview, IDE, and VNC/browser control in one product. Parallel Code remains primarily terminal-centric.
   Priority: `High`
   Difficulty: `Large`

4. **Workspace/sandbox abstraction with security and resource policy**
   AgentRove has more explicit workspace ownership, sandbox provider selection, preview restrictions, and resource-aware backend orchestration. Parallel Code has per-task worktrees, but not the same execution abstraction.
   Priority: `Medium`
   Difficulty: `Large`

5. **Preview polish and richer runtime compatibility**
   Parallel Code now has explicit task-scoped preview exposure, but AgentRove still goes further with mobile preview, broader preview/workbench integration, and a sandbox-backed ownership model.
   Priority: `High`
   Difficulty: `Medium`
...
- Parallel Code is ahead in **multi-agent worktree isolation**, **mobile remote monitoring**, and **terminal transport quality**.
...
| Project-level diff viewer           | Repo-wide diff endpoint with `all`, `staged`, `unstaged`, and `branch` modes. Unified and split rendering in a dedicated view.          | `ChangedFilesList` plus `DiffViewerDialog` for single-file inspection. Good file-level diff, no repo-wide review mode.                                 | Large review workflow gap.                                                                                                             | High     | Medium     |
...
| Security boundary around previews   | Preview exposure is tied to sandbox ownership and excluded-port rules.                                                                  | Parallel Code now has explicit task-level exposure and auth-gated preview proxying, but still relies on host-mode trust rather than sandbox ownership. | Preview exposure is safer now, but still weaker than a true sandbox-backed boundary.                                                   | Medium   | Large      |
...
- If an agent starts a Vite, Next.js, or Expo dev server inside a task, Parallel Code can detect the likely port, let the user expose it explicitly, and preview it through the product.
- The remaining gap is breadth and polish, not absence.

#### Why this matters

- It blocks a browser-mode workflow: "ask the agent to run the app, then inspect the result without leaving the product"
- It makes Parallel Code weaker for frontend and full-stack tasks than for pure CLI workflows
...
3. Improve preview compatibility and polish for root-relative assets, HMR, and mobile-preview workflows.
...
- Parallel Code runs tasks on the host, not in a strict sandbox. AgentRove-style preview parity remains harder because the app must avoid proxying unrelated localhost services.

### 2. Multi-File Project-Level Review
...
- `ChangedFilesList` gives a per-task list of modified files
- `DiffViewerDialog` renders single-file diffs
- Arena and merge flows reuse the same changed-files and per-file diff patterns
- No dedicated repository-wide diff screen, no staged/unstaged/branch review modes, and no dedicated project browser
...
- Parallel Code is already oriented around multi-agent code generation.
- Review quality matters more as generated changes increase.
...
- It strengthens Parallel Code's existing multi-agent and merge workflows.
...
- This matters for enterprise use or any environment where the browser server is shared across devices.
...
This is less work than full preview parity and improves safety.
...
- Embedded IDE access
- VNC/browser surfaces for debugging and app inspection
- A workbench that combines those surfaces
...
- This moves the product beyond terminal management.
- For UI-heavy tasks, code, preview, and inspection in one window are a clear advantage.
...
- Without stronger execution isolation, embedded browser/VNC parity is expensive and risky.
...
- Several AgentRove features are not isolated UI widgets. They come from a more explicit execution model.
...
This is foundational work, but it should follow the higher-ROI UX features unless Parallel Code plans a broader platform shift.

## Features Where Parallel Code Is Ahead

### 1. Multi-Agent Git Worktrees and Merge Flow

Parallel Code has a clearer "one task, one branch/worktree" operating model than AgentRove.
...
Parallel Code's browser/mobile companion is more complete for supervising agents from a phone or second device.
...
Parallel Code appears better optimized for long-lived remote terminal usage.
...
AgentRove has a broader workspace/sandbox model, but it also carries more backend complexity. Parallel Code is simpler to run locally and has a remote mode without requiring the same server stack.

Preserve that simplicity while adding parity features.
...
1. **Project-level diff/review panel**
   Highest near-term value. Builds directly on existing git and diff infrastructure.
...
5. **Execution profile / sandbox abstraction**
   Foundation for safer previewing and embedded tooling.
...
If the goal is to close the most obvious competitive gaps without losing Parallel Code's strengths, the near-term plan is:

- ship a **repo-level review workspace**
- add **interactive permission approvals**
- add **task-scoped app preview URLs**

Those three changes would address the most visible shortcomings versus AgentRove while preserving what Parallel Code already does better: multi-agent branch isolation, remote supervision, and terminal ergonomics.
