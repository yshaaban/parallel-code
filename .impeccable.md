...
- A visible keyboard-focus perimeter on every interactive surface, including forced-color modes
- Static equivalents for status and acknowledgement cues when reduced motion is requested
...
6. Treat motion as progressive enhancement. Respect the current reduced-motion preference without
   hiding state or stopping functional progress indicators.
